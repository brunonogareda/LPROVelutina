...
int opcion;
 ...
 int main( int argc, const char** argv )
 {
   ...
   Mat img, frame, Mask, mascara, original, lap, frame_gray;
...
int i =0;
   for (;;) {
       ...
       if( !frame.empty() )
       {
           ...
           if(puntos.size()!=0){
            cout << "Numero de rectangulos: " << puntos.size() << endl;
            waitKey();
           }
            }
           else{
       ...
       case 'c':
        ...
        case 'k':
           ...
           Laplacian(frame_gray, lap, CV_16S, 3, 1, 0, BORDER_DEFAULT);
...
vector<Point2i> histograma(vector<Rect> boundRect, Mat frame){
    ...
    for (int i=0; i<boundRect.size();i++){
        ...
        inRange(rectSectionHSV, Scalar(0,150,150), Scalar(40,255,255),rectInterested);
        ...
        inRange(rectSectionHSV, Scalar(25,0,0), Scalar(75,255,255),verde);
        //erode(rectInterested, rectInterested, getStructuringElement(MORPH_ELLIPSE, Size(5,5)));
        findContours(verde, contours2, CV_RETR_LIST, CV_CHAIN_APPROX_SIMPLE);
        inRange(rectSectionHSV, Scalar(115,150,127), Scalar(150,255,255),azul);
        //erode(rectInterested, rectInterested, getStructuringElement(MORPH_ELLIPSE, Size(5,5)));
        findContours(azul, contours3, CV_RETR_LIST, CV_CHAIN_APPROX_SIMPLE);
        if(contours2.size()!=0&&(contours.size()!=0||contours3.size()!=0)){
        ...
        */
        rectSection=frame(boundRect[i]);
        cvtColor(rectSection, rectSectionHSV, COLOR_BGR2HSV);
        //inRange(rectSectionHSV, Scalar(0,54,160), Scalar(52,123,255),rectInterested);
    ...
    for(int i=0; i<amarillo.size(); i++){
        ...
        rectangle(frame, amarillo[i], 100, 1, 8, 0);
    }
    resize(frame, frameAux, Size(), 0.5, 0.5, INTER_LINEAR);
...
Point2i angulo(Rect coord, Mat frame){
    ...
    float areaframe = (frame.cols)*(frame.rows);
    float relacion = area/areaframe;
    float distancia;
    float area_objeto=28;
    float k=290485.47;
   // float distancia = log(1/(relacion*relacion))/10;
    //cout << "Dimensiones: " << coord.width << "x" << coord.height << endl;
    distancia=sqrt((area_objeto*k)/area);
    cout << "Area de la avispa: " << area << endl;
    //cout << "Area del frame: " << areaframe << endl;
    //cout << "Espacio ocupado: " << relacion << endl;
...
while(true){
     ...
     mascara.copyTo(frameAux2,imgThresholded);
     resize(frameAux2, frameAux2, Size(), 0.5, 0.5, INTER_LINEAR);
     imshow("Imagen2", frameAux2); //show the thresholded image
     imgHSV.copyTo(frameAux,imgThresholded);
     resize(frameAux, frameAux, Size(), 0.5, 0.5, INTER_LINEAR);
     imshow("Imagen", frameAux); //show the thresholded image
     resize(imgThresholded, imgThresholded, Size(), 0.5, 0.5, INTER_LINEAR);
...
vector<Point2i> hist_no_mov(Mat frame){
    ...
    //resize(frameHSV, frameHSV, Size(), 0.5, 0.5, INTER_LINEAR);


    //cvtColor(frame, frameHSV, COLOR_BGR2HSV);
    //inRange(rectSectionHSV, Scalar(0,54,160), Scalar(52,123,255),rectInterested);
    inRange(frame, Scalar(90,192,117), Scalar(112,255,217),rectInterested);
        erode(rectInterested,rectInterested, getStructuringElement(MORPH_ELLIPSE, Size(5,5)));
        dilate(rectInterested,rectInterested, getStructuringElement(MORPH_ELLIPSE, Size(11,11)));
        resize(rectInterested, rectInterestedMini, Size(), 0.5, 0.5, INTER_LINEAR);
        imshow("mascara", rectInterestedMini);
